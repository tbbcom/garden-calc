// Garden Calculator - External JavaScript File
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.unit-ibtn').forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (unit === 'imperial') {
                if (labels[0]) labels[0].textContent = '(feet)';
                if (labels[1]) labels[1].textContent = '(feet)';
                if (labels[2]) labels[2].textContent = '(inches)';
            } else {
                if (labels[0]) labels[0].textContent = '(meters)';
                if (labels[1]) labels[1].textContent = '(meters)';
                if (labels[2]) labels[2].textContent = '(centimeters)';
            }
        });
    });
    ...
    window.calculateSoil = calculateSoil;
    window.calculateMulch = calculateMulch;
    window.calculateSpacing = calculateSpacing;
    window.calculateWatering = calculateWatering;
    window.calculateCompost = calculateCompost;
